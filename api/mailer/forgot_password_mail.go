...
func (s *sendMail) SendResetPassword(ToUser string, FromAdmin string, Token string, Sendgridkey string, AppEnv string) (*EmailResponse, error) {
	...
	if os.Getenv("APP_ENV") == "production" {
		forgotUrl = "https://forum.glugate.com/resetpassword/" + Token //this is the url of the frontend app
	} else {
	...
	email := hermes.Email{
		Body: hermes.Body{
			Name: ToUser,
			Intros: []string{
				"Welcome to Glugate Forum! Good to have you here.",
			},
	...
	from := mail.NewEmail("Glugate Forum", FromAdmin)
